...
import (
	...
	"github.com/summerKK/go-code-snippet-library/cmd-tools/internal/timer"
)
...
var calculateTimeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if calculateTime == "" {
			...
			// 这里是假设`calculateTime`是时间格式(不是时间戳),看是否能转换.
...
func init() {
	...
	calculateTimeCmd.Flags().StringVarP(&duration, "duration", "d", "0s", "持续时间,有效时间单位为:ns,us,ms,s,m,h")
}
